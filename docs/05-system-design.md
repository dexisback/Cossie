...
- **Prompt injection logged, not blocked.** Detection favors observability over aggressive prevention: many legitimate users discuss injection techniques for security research, education, debugging and documentation, and automatic blocking would cause false positives. Suspicious prompts are detected, classified, logged and surfaced to administrators; execution proceeds normally. Prompt injection becomes an observable security signal rather than an automatic failure, informing administrators without restricting legitimate use cases.
...
| Prompt logging vs prompt blocking | Weaker preventative security | Fewer false positives; visibility without restricting legitimate usage |
